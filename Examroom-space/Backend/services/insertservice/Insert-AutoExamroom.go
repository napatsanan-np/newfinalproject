...
func (s *UseInsertService) NewLect(date string, time string, selectroom []string) (*ExamAllocator, error) {
	...
	if err := ea.loadRooms(selectroom); err != nil {
...
func (ea *ExamAllocator) loadRooms(slectroom []string) error {
	var query string
	var rows *sql.Rows
	var err error
	...
	if len(slectroom) == 0 {
		query = `
SELECT room_id, room_name, room_type, capacity FROM rooms  
WHERE room_type = 'ห้องบรรยาย' 
ORDER BY capacity;
		`
		...
		query = `
SELECT room_id, room_name, room_type, capacity FROM rooms  
WHERE room_id NOT IN (` + placeholders + `) AND room_type = 'ห้องบรรยาย' 
ORDER BY capacity;
		`
...
func (ea *ExamAllocator) loadExams(date string, time string, id_config int) error {
	...
	if date != "" {
		query += fmt.Sprintf(" AND edate = $%d", len(args)+1)
		args = append(args, date)
	}
	if time != "" {
		query += fmt.Sprintf(" AND etime = $%d", len(args)+1)
	...
	log.Println("exam::", ea.Exams)
	return nil
}

func (ea *ExamAllocator) findBestRoom(studentCount int, examDate string, startTime string, duration string, exam models.RoomExam, ref int, course string) (models.Room, bool) {
...
func calculateEndTime(startTime string, duration string) (string, error) {
	startHour := startTime[:2]
	startMin := startTime[3:5]

	h, err := strconv.Atoi(startHour)
	if err != nil {
		return "", err
	}
	m, err := strconv.Atoi(startMin)
	if err != nil {
		return "", err
	}

	durationHours, err := strconv.Atoi(duration)
	if err != nil {
		return "", err
	}

	endTimeMinutes := h*60 + m + durationHours*60
	endHour := endTimeMinutes / 60
	endMin := endTimeMinutes % 60
...
func (ea *ExamAllocator) isRoomAvailable(roomID string, edate string, etime string, no_st int, ref int, exam models.RoomExam, Capacity int, Hr string) bool {
	...
	if hasRows {
		sql := `SELECT SUM(CAST(num_st AS INT)) AS num_st FROM public.roomexam re inner join rooms r on r.room_id = re.room_id where r.room_id = $1  and id_config = $2;`
		var totalStudents int
		err := ea.db.QueryRow(sql, roomID, config[0].Id_config).Scan(&totalStudents)
		if err != nil {

		}
		log.Println("REF::", ref, "room_id", roomID, ref, "nost", no_st, "Total", totalStudents, no_st+totalStudents, "<", Capacity)
		return (no_st+totalStudents < Capacity)
	}
	//	fmt.Println("REF::", ref, "found", roomID)
	return !hasRows
}

func (ea *ExamAllocator) InsertRoomWithRef(bestRoom models.Room, exam models.RoomExam) {
	// Allocate room
	config, _ := ea.GetConfig()
	tx, err := ea.db.Begin()
	if err != nil {
...
func (ea *ExamAllocator) getExamsByRef(ref int) ([]models.RoomExam, error) {
	config, _ := ea.GetConfig()
	query := `
...
func (ea *ExamAllocator) allocateRooms(tx *sql.Tx, exams []models.RoomExam, roomAllocation []RoomAllocation) error {
...
func (ea *ExamAllocator) AllocateRooms() (*AllocationResult, error) {
	tx, err := ea.db.Begin()
	if err != nil {
	...
	for _, exam := range ea.Exams {
		...
		studentCount, err := strconv.Atoi(exam.Num_st)
		if err != nil {
			log.Println("Error converting Num_st to int:", err)
		} else {
			log.Println("REF:::", exam.Ref, "studentCount:", studentCount)
		}
		...
		//log.Println("Ref::", exam.Ref, "Bestroom", bestRoom, "STD_COUNT", studentCount)
		if !found {
...
func (i *UseInsertService) ClearDataroom() {
	// Allocate room
	tx, err := i.DB.Begin()
	if err != nil {
		fmt.Errorf("failed to start transaction: %w", err)
...
func (ea *ExamAllocator) GetConfig() ([]models.ExamConfig, error) {
	var jsonData []byte
